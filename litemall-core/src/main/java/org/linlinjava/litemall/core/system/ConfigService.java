...
class ConfigService {
    ...
    public void reloadConfig(String prefix) {
        List<TicSystem> list = litemallSystemConfigService.queryAll();
        for (TicSystem item : list) {
            //符合条件，添加
            if (item.getSysKeyName().startsWith(prefix))
                BaseConfig.addConfig(item.getSysKeyName(), item.getSysKeyValue());
        }
    }
