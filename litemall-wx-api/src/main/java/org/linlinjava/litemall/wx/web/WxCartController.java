...
public class WxCartController {
     ...
     */
    @GetMapping("checkout")
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer couponId, Integer grouponRulesId) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        ...
        System.out.println(grouponRulesId);
        // 团购优惠
        ...
        List<TicCouponUser> couponUserList = couponUserService.queryAll(userId);
        for(TicCouponUser couponUser : couponUserList){
            TicCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCosCouponId(), checkedGoodsPrice);
            if(coupon == null){
                continue;
            }

            tmpCouponLength++;
            if(tmpCouponPrice.compareTo(coupon.getCouDiscount()) == -1){
                tmpCouponPrice = coupon.getCouDiscount();
                tmpCouponId = coupon.getCouId();
            }
        }
        // 获取优惠券减免金额，优惠券可用数量
        ...
        if (couponId == null || couponId.equals(-1)){
            ...
            TicCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, checkedGoodsPrice);
            // 用户选择的优惠券有问题
            if(coupon == null){
                return ResponseUtil.badArgumentValue();
            }
            couponPrice = coupon.getCouDiscount();
        }
